...
function* watchInitFetchUser() {
  yield takeEvery(ActionTypes.INIT_FETCH_USER, function* ({ username }) {
    try {
      ...
      const { pk } = userList.find((u) => u.username === username);
      yield put(actions.fetchUser({ username, pk }));
    } catch (e) {
      console.log(e);
    }
  });
...
/**
 * @description 유저 검색 후 클릭했을 때 사용하는 함수
 */
function* watchFetchUser() {
  yield takeEvery(ActionTypes.FETCH_USER, function* ({ pk }) {
    yield put(actions.requestFetchUser(pk));
    try {
      const [userInfoData, userStoryData] = yield all([
        call(services.getUserInfo, pk),
        call(services.checkUserStory, pk),
      ]);
...
export default function* userWatcher() {
  yield all([fork(watchFetchUser), fork(watchInitFetchUser)]);
}
